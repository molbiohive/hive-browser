...
class TestBatchFanOut:
    ...
    def _tool_call_response(self, tool_name, arguments, call_id="call_1"):
        return {
            "choices": [{
                "message": {
                    "content": None,
    ...
    async def test_batch_sequences(self):
        """Tool tagged batch fans out sequences into individual calls."""
        ...
        class GcTool(Tool):
            ...
            widget = "text"
            tags = {"llm", "batch"}
            params = {"sequence": {"type": "string", "description": "DNA", "required": True}}
            ...
            async def execute(self, params):
                ...
                return {"gc_percent": gc, "length": len(seq)}

        reg = ToolRegistry()
        reg.register(GcTool())

        llm = self._mock_llm([
            self._tool_call_response("gc", {"sequences": ["ATGC", "GGCC"]}, call_id="c1"),
            self._text_response("GC content: 50% and 100%."),
        ])
        resp = await route_input("gc for ATGC and GGCC", reg, llm_client=llm)
        assert resp["type"] == "tool_result"
        data = resp["data"]
        assert data["count"] == 2
        assert len(data["results"]) == 2
        assert data["results"][0]["gc_percent"] == 50.0
        assert data["results"][1]["gc_percent"] == 100.0
        assert data["results"][0]["_label"] == "ATGC"

    async def test_batch_sids(self):
        """Batch fan-out works for sid -> sids."""

        class ProfileTool(Tool):
            name = "profile"
            description = "Show sequence details"
            widget = "profile"
            tags = {"llm", "batch"}
            params = {"sid": {"type": "integer", "description": "Sequence ID", "required": True}}

            def __init__(self, **_):
                pass

            async def execute(self, params):
                return {"sid": params["sid"], "name": f"seq_{params['sid']}"}

        reg = ToolRegistry()
        reg.register(ProfileTool())

        llm = self._mock_llm([
            self._tool_call_response("profile", {"sids": [1, 2, 3]}, call_id="c1"),
            self._text_response("Profiles for 3 sequences."),
        ])
        resp = await route_input("show profiles", reg, llm_client=llm)
        assert resp["data"]["count"] == 3
        assert resp["data"]["results"][2]["sid"] == 3

    async def test_non_batch_tool_ignores_plural(self):
        """Tool without batch tag does not fan out."""

        class SearchTool(Tool):
            name = "search"
            description = "Search"
            widget = "table"
            tags = {"llm"}  # no "batch" tag
        ...
        llm = self._mock_llm([
            self._tool_call_response("search", {"query": "GFP"}, call_id="c1"),
            self._text_response("No results."),
        ])
        resp = await route_input("find GFP", reg, llm_client=llm)
        assert resp["type"] == "tool_result"
        # No batch wrapper
...
# ── Batch Schema Injection ──


class TestBatchSchema:
    def test_batch_tag_adds_plural_params(self):
        ...
        class BatchTool(Tool):
            ...
            tags = {"llm", "batch"}
            params = {
                "sequence": {"type": "string", "description": "DNA"},
        ...
        schemas = build_tool_schema([BatchTool()])
        props = schemas[0]["function"]["parameters"]["properties"]
        # sequence is a known batch param -> sequences added
        assert "sequences" in props
        assert props["sequences"]["type"] == "array"
        # format is NOT a known batch param -> formats NOT added
        assert "formats" not in props

    def test_no_batch_tag_no_plurals(self):
        from hive.llm.prompts import build_tool_schema

        class NoBatchTool(Tool):
            name = "search"
            description = "Search"
            tags = {"llm"}
        ...
        schemas = build_tool_schema([NoBatchTool()])
        props = schemas[0]["function"]["parameters"]["properties"]
        assert "sequences" not in props
